class EmojisParser {
	...
	#ignoreOtherRequestsOp;

	parse(textElement) {
		if (typeof this.#ignoreOtherRequestsOp == 'undefined') {
		...
		if (typeof textElement == 'string') {
			content = utils.escapeHTML(textElement).trim();
		...
		for (const match of allMatch) {
			const unifiedCode = this.#nativeToUnified(this.#removeVS16s(match[0]));
		...
		if (allMatch.length) {
			...
			for (const emoji of element.querySelectorAll('emoji')) {
				...
				imageElement.src = '/emoji-data/img-apple-60/img-apple-64/'+emoji.id+'.png';
...
const emojisParser = new EmojisParser();